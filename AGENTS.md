...
- Both scripts hardcode a `TOOLS`/`$Tools` list of the top-level commands to make runnable (`ffinpaint`, `ffrife`, `ffv`, `jwdl`, `jwpl`, `jwvideo-mux`, `slverse`). **Add new tools to that list in both scripts** when they're added to the repo, or the installer won't make them executable/shimmed.
