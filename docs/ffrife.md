...
- **Streaming encode**: when chunking is active, `ffrife` no longer keeps every interpolated frame on disk until one whole-clip encode at the end. Instead, every `stream_segment_chunks=8` completed RIFE chunks (`--segment-chunks`) are hardlinked into a temporary flat directory and encoded straight to their own small AV1 (or configured codec) segment - only then are those chunks' PNGs freed, so peak disk for the interpolated side is bounded to roughly one batch's worth instead of the whole clip. Segments are joined losslessly (`-c copy`, no re-encode) into one file once every chunk is done. This works internally in `.mkv` (Matroska tolerates truncation from an interrupted process far better than MP4's end-of-file index does) and is delivered as whatever container `-o`/`--container` actually asked for via one more lossless `-c copy` remux - transparent to the output, but means a run targeting `.mp4` still uses a `.mkv` working file under the hood until the very last step. Lower `--segment-chunks` for a tighter disk bound at the cost of a few more small forced keyframes at segment boundaries (imperceptible under CRF - it costs a little more file size, not quality) *and* a real, measured wall-clock cost: each extra segment is a separate ffmpeg/encoder process, and on a short clip that per-process startup overhead can dominate (measured ~2s/segment on a 320x240 synthetic clip, regardless of codec preset speed - see the benchmark in the repo history around this feature's introduction). That overhead is fixed per segment, not per frame, so it matters far less against RIFE's own GPU inference time on real, longer footage (RIFE is "by far the slowest step in the whole pipeline" per the benchmark above) - but it's a real trade-off, not a free lunch. Higher `--segment-chunks` for the reverse (closer to the old whole-clip peak, fewer segment-encode invocations). A crash only costs the batch that was still in progress: earlier segments are validated (they decode cleanly) and reused as-is on resume, never regenerated from scratch. Source frame extraction itself still happens as one upfront pass, since scene-cut/transition detection (below) needs the whole clip's source frames available regardless.
...
- PNG extraction and RIFE itself are lossless intermediate steps; the only lossy encoding happens going from PNG frames to video, once per segment (see "Streaming encode" above) rather than in one pass over the whole clip.
- Source-frame extraction is still one upfront pass, so its PNGs are a fixed, bounded cost proportional to the clip's own source frame count - not a spike. Interpolated-frame PNGs no longer accumulate for the whole clip: with chunking active (see "Streaming encode" above), each `stream_segment_chunks` batch is encoded and freed before the next one starts, so peak disk for that side is roughly one batch's worth, not the whole output frame count. `--no-space-check` skips the preflight estimate if it's ever wrong for your content; intermediate work is cleaned after success and retained (with its location printed) after an interrupted job either way.
