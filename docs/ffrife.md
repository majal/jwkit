...
- Gradual transition detection is also enabled by default. It analyzes 32×18 grayscale frames and conservatively identifies sustained, directionally consistent pixel changes typical of fades and dissolves (`transition_min_duration=0.25`, `transition_sensitivity=1.35`, `transition_alignment=0.80`). RIFE-generated samples inside those intervals are replaced with holds of the preceding real source frame, preserving the authored transition without asking RIFE to infer motion between blended shots. Use `--no-gradual-transition-detection` for hard cuts only. For a transition the heuristic misses—or unusual continuous motion it mistakes for one—`--transition-ranges START:END,...` is the deterministic override in seconds.
