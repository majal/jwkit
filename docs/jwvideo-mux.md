...
A bare filename (no `CODE=` prefix) works too, as long as it follows the same `{prefix}_{CODE}_...` naming convention as the base file -- its language is inferred from the filename itself:

```bash
./jwvideo-mux S-319-27v_FSL_01_r720P_rife.mp4 -v FSL -a S-319-27v_HV_01_r240P.mp4
```
...
- A code may also be a bare `path/to/file` with no `CODE=` prefix (again, local file mode only): its language is inferred from the filename by checking whether it shares the base video's own `{prefix}_{CODE}_...` naming convention. This is a convenience over the explicit form above, not a guess-at-all-costs feature -- a filename that doesn't share the base's prefix (or when the main input isn't a local file at all, so there's no base to compare against) is a clean error telling you to use `CODE=path` instead.
