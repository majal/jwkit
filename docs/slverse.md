...
- Reference-language detection first compares a single source marker's exact normalized caption with the replacement caption; an exact match skips delogo/redraw automatically. `sign_lang_ref_language` remains the fallback for multi-verse ranges and spelling/abbreviation differences that cannot be inferred safely from one caption. `font_weight_estimator` also remains necessary: dynamic sizing measures the known marker text in a configurable proxy font; it does not yet detect the source video's actual glyph pixels.
