...
- `-s`/`--trim-start` and `-e`/`--trim-end` take positive trim amounts — e.g. `-s 5.302` skips the first 5.302s and `-e 3` ends 3s early. The old signed `-e -3` spelling remains accepted; explicit `--offset-start`/`--offset-end` retain signed-offset semantics for compatibility and unusual advanced use.
...
Every `slverse config list` setting has a matching `--<dashed-key>` flag on `extract`/`bulk` for a one-run override (e.g. `--delogo-engine inpaint`, `--overlay-fontsize 40`, `--cache-max-gb 2`) - `--encoder`/`--codec`/`--crf`/`--preset` above are just the ones with a shorter, dedicated name. Run `slverse extract --help` for the full list.
