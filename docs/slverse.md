...
- Preview mode (`extract` without `--write`) plays from a locally cached copy of the chapter when one already exists under `cache_dir` (from a prior `--cache` extract or a `bulk` run) instead of streaming the remote URL - faster and works offline. The remote URL is still printed either way.
