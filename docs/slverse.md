...
- `slverse find <book> <chapter> <verse>` searches every language you've already synced for who has a given verse, without downloading anything. `<verse>` also accepts a range (`25-27`) or comma list (`1,3,5`). Default output is a compact ✅/❌ table grouped by language; `--verbose` shows full details, and `--json` gives scripts a machine-readable array. `--play` opens one paused window per language and contiguous requested run, not one per verse: `ffv all Matthew 13:16,17 -p`, for example, opens four two-verse windows when four languages have both verses. A missing or unrequested verse always breaks a run, so a combined preview never spans it silently. Segment downloads run in parallel and each window opens as soon as its own segment is ready; `preview_max_parallel`/`--preview-max-parallel`/`-j` controls the bound (default 4).
...
| `preview_max_parallel` | `4` | Maximum simultaneous segment downloads for `find --play` / `ffv all ... -p`. Override once with `-j N` or `--preview-max-parallel N`. |
...
- Extraction defaults to `extract_mode = segment`: it stream-copies and caches just the requested verse window while preserving the chapter timestamps used by downstream trimming and overlays. If the segment fetch fails it safely falls back to the remote source. Use `--cache` when extracting several different verses from one chapter, where one verified full-chapter download is more efficient. Cached files live under `cache_dir` (default `~/.cache/slverse/`), capped at `cache_max_size` (default `1G`) with segments and chapters sharing the same LRU budget. Pass `--onthefly` only when you explicitly want no disk cache.
...
- Preview mode (`extract` without `--write`) and `find --play` both default to `preview_source = segment`: they fetch and cache the exact requested marker window (a contiguous multi-verse request is one combined file), keyed by language, book, chapter, start, and end. Thus a combined 16-17 cache entry cannot be confused with either single-verse entry, while the sync index remains the per-verse source of truth for boundaries. Set `preview_source = cache` to instead download and reuse the whole chapter, or `preview_source = remote` to stream without disk. `find --play` resolves up to `preview_max_parallel` segments concurrently (default 4; `-j`/`--preview-max-parallel` overrides one run) and opens each completed language window while other downloads continue.
...
- `find --play`'s multi-language preview windows are sized via `preview_window_size` (default `65%` of screen size, aspect preserved, via mpv's `--autofit`) when previewing more than one language/contiguous run; a single window still autoplays fullscreen.
