...
- `--slow`/`--fast` split the extracted verse into alternating normal/retimed sections at one or more boundary times, the same sectioned retiming the standalone `ffslow`/`fffast` tools do (`--slow` retimes the 2nd/4th/... section, `--fast` the 1st/3rd/...) - with `interpolation_engine = rife`, the retimed sections get RIFE-interpolated first via `ffrife` for smooth (not choppy) slow-motion. If a source has audio, the matching audio pieces are pitch-preserving retimed and concatenated too.
...
- Resolves book names case-insensitively and without requiring diacritics. Set `api_language` to a priority list such as `VT,E` to accept Vietnamese names first and fall back to English.
...
- `slverse find <book> <chapter> <verse>` searches every language you've already synced for who has a given verse and, by default, also opens a preview of each match - pass `--list-only`/`-l` to just list without previewing. `<verse>` also accepts a range (`25-27`) or comma list (`1,3,5`). Default output is a compact ✅/❌ table grouped by language; `--verbose` shows full details, and `--json` gives scripts a machine-readable array. Previewing opens one paused window per language and contiguous requested run, not one per verse: `ffv all Matthew 13:16,17`, for example, opens four two-verse windows when four languages have both verses. A missing or unrequested verse always breaks a run, so a combined preview never spans it silently. Segment downloads run in parallel and each window opens as soon as its own segment is ready; `preview_max_parallel`/`--preview-max-parallel`/`-j` controls the bound (default 4). `--play`/`-p` still works but is a no-op now that previewing is the default.
...
- `-s`/`--trim-start` and `-e`/`--trim-end` differ from `ffrife`'s absolute timestamps: these are non-negative amounts removed from the natural verse window's front and tail. They accept `SS.sss`, `MM:SS.sss`, or `HH:MM:SS.sss` — e.g. `-s 1:05.302` skips the first 65.302s. Negative values are rejected.
- `--window START-END` is the compact form for one relative slice: `--window 1:03.567-1:10.003` keeps exactly that interval within the natural verse window. The older seconds-only `3.567:10.003` form remains accepted.
- Output filenames compact contiguous verse ranges (`Rev_13_1-3_ASL.mp4`) and append the meaningful edits: `_cut`, `_slow`, `_fast`, and the enabled interpolation engine (`_rife`, `_minterpolate`, or `_framerate`). Combined edits remain ordered and readable, such as `Rev_13_1-3_ASL_cut_slow_rife.mp4`. An explicit `-o/--output` is always used exactly as supplied. Files also include provenance/edit metadata and carry seekable MP4 chapters for each included verse.
...
- `mpv` (optional, only for `slverse find`'s preview - or `--play` on `extract`)
...
| `interpolation_engine` | `rife` | Engine used when interpolation is enabled: `rife`, `minterpolate`, or `framerate`. Use `interpolate` to enable or disable it. |
| `rife_scene_detection` | `false` | Detect hard cuts before RIFE. Verse clips are continuous, so this is off here even though standalone `ffrife` defaults it on. Override with `--rife-scene-detection` or `--no-rife-scene-detection`. |
| `smooth_slow_motion` | `true` | Synthesize extra frames in `--slow` sections even when whole-clip interpolation (`-i`) is off. Override with `--smooth-slow-motion` or `--no-smooth-slow-motion`. |
...
| `min_transition_duration` | `0.5` | Seconds below which a marker's `endTransitionDuration` is treated as routine per-verse padding rather than a real fade — JW tags nearly every verse with a small nonzero value (empirically 0.000–0.300s in a real index) distinct from an actual paragraph/chapter fade (empirically 1.0s+). Below this floor, `trim_end_transition`/`trim_mid_transitions` don't cut it and the verse-reference overlay doesn't try to fade through it either — a fade shorter than a single video frame otherwise reads as a one-frame flash where the reference (and the small source-SL label, since it fades on the same schedule) both disappear. |
...
| `preview_max_parallel` | `4` | Maximum simultaneous segment downloads for `find`'s preview / `ffv all ...`. Override once with `-j N` or `--preview-max-parallel N`. |
| `api_language` | `E` | Priority-ordered book-name metadata languages. Use `VT,E` for Vietnamese names with English fallback; matching ignores case and diacritics. |
| `bible_book_path_overrides` | _(empty)_ | `MEPS_CODE=url-path` manual escape hatch — normally unnecessary. jw.org translates each language edition's whole Study Bible "Books" page path (not just the leading locale segment), but every locale's own homepage embeds a `data-bible_data_api` attribute pointing at that language's API — the same mechanism jw.org's own site JS uses — so the correct path is auto-discovered per language and cached in `state.json`, with no per-language setup needed. Only add an entry here if a language you set in `api_language` fails to resolve despite having a real Study Bible edition: open that language's own jw.org Study Bible "Books" page and copy everything after the locale segment. |
| `delogo_width_pad` / `delogo_height_pad` | `2` / `2` | Small antialiasing margin beyond the measured source-caption proxy. Two pixels avoids the fragility of a 1px edge while reducing the chance of touching a nearby hand. |
| `caption_detection` / `caption_detection_samples` | `true` / `3` | Measure the static near-white source caption from small upper-left frame crops. Three interior samples are majority-voted; uncertain detection falls back to proxy-font sizing. Disable once with `--no-caption-detection`, or change the 1-9 sample count with `--caption-detection-samples`. |
...
Config lives in `~/.config/jwkit/slverse/config.toml`. Use `config list|get|set|path|edit|reset|diff|check`; unknown keys and invalid values are rejected by `config set`, while `config check` also catches bad hand edits, malformed mpv quoting, and missing configured executables.
...
Every `slverse config list` setting has a matching `--<dashed-key>` flag on `extract`/`bulk` for a one-run override (e.g. `--delogo-engine inpaint`, `--overlay-fontsize 40`, `--cache-max-size 2G`) - `--encoder`/`--codec`/`--crf`/`--preset` above are just the ones with a shorter, dedicated name. Run `slverse extract --help` for the full list.
...
- Extraction defaults to `extract_mode = segment`: on a cache miss it stream-copies and caches just the requested verse window while preserving the chapter timestamps used by downstream trimming and overlays. If the fetch fails it safely falls back to the remote source. Every cache-aware request - `extract` preview, `extract --write` (`segment`/`cache` modes), and `find`'s preview - shares one interoperable cache per chapter (`cache_dir/<lang>/<book>_<chapter>/`): a whole chapter, a verse range, or a single verse can all satisfy a *later, narrower* request that falls inside an already-cached window, regardless of which command or mode originally fetched it. Use `--cache` when extracting several different verses from one chapter, so one full-chapter download up front amortizes across all of them - it never becomes the default just because interoperable reuse exists. Cache entries are keyed by the source's own checksum (jw.org's pub-media API), not by filename/URL, so a still-in-progress chapter that gets new or re-recorded verses under the same URL has its stale cached bytes purged and replaced automatically the next time that chapter is used, rather than serving pre-update content indefinitely. Cached files live under `cache_dir` (default `~/.cache/slverse/`), capped at `cache_max_size` (default `1G`) with segments and whole chapters sharing the same LRU budget. Pass `--onthefly` only when you explicitly want no disk cache.
...
- `slverse` sets `rife_scene_detection = false` by default because a verse clip is one continuous shot. Use `--rife-scene-detection` for unusual source material with cuts; standalone `ffrife` keeps detection enabled by default for arbitrary videos.
- Interpolation enablement is separate from engine selection: users default to `interpolate = false` and `interpolation_engine = rife`. Use `--interpolate` or `--no-interpolate` for one run and `-I`/`--interpolation-engine` to select the engine.
...
- Reference-language detection first compares a single source marker's exact normalized caption with the replacement caption; an exact match skips delogo/redraw automatically. `sign_lang_ref_language` remains the fallback for multi-verse ranges and spelling/abbreviation differences that cannot be inferred safely from one caption.
- Delogo sizing normally samples three interior frames and analyzes only a 500x105 crop around the known upper-left caption position. It detects persistent near-white, low-chroma glyph pixels without OCR or an image-library dependency, then uses the median box only when a majority agrees. On tested local 720p segments this costs about 0.2 seconds for three samples; the byte analysis itself is about 7ms per frame and FFmpeg startup/decoding is the rest. `font_weight_estimator` remains the automatic fallback for fades, unusual backgrounds, decode failures, or disagreeing samples.
...
- `--slow`/`--fast` need `--write`/`-f` (no live preview for sectioned retiming); boundary times accept `SS.sss`, `MM:SS.sss`, or `HH:MM:SS.sss`, relative to the verse's own start, and must be strictly increasing and inside the verse's duration. `--speed` defaults to `0.5` for `--slow` (half speed / 2× duration) and `3` for `--fast`. Slow sections synthesize their full retimed frame count by default even without `-i`; disable that with `--no-smooth-slow-motion`. RIFE sections remain lossless PNG sequences until concat/overlay, while untouched sections feed directly from the source; the completed clip gets one video encode rather than encoding temporary sections. If the source has audio, each matching audio section is trimmed, zero-based, pitch-preserving `atempo`-retimed, and concatenated beside the video. RIFE absence follows `ffrife`'s existing fallback preference.
- Preview mode (`extract` without `--write`) and `find`'s preview both default to `preview_source = segment`: on a cache miss they fetch just the requested marker window (a contiguous multi-verse request is one combined file), but any request first checks the same per-chapter cache `extract --write` uses (see above) for an already-cached window - whole chapter, a range, or another verse - that covers what's being asked for, so a *different* verse range from an already-cached chapter is never a cache miss just because it doesn't match a prior request byte-for-byte. Set `preview_source = cache` to additionally download and reuse the whole chapter on a miss, or `preview_source = remote` to always stream without disk. `find`'s preview resolves up to `preview_max_parallel` segments concurrently (default 4; `-j`/`--preview-max-parallel` overrides one run) and opens each completed language window while other downloads continue.
- RIFE interpolation honors `--trim-mid-transitions`: each kept range stays lossless through interpolation and all ranges join in the one final encode. `--slow`/`--fast` combined with mid-transition trimming remains unsupported and keeps those transitions.
...
- `find`'s multi-language preview windows are sized via `preview_window_size` (default `65%` of screen size, aspect preserved, via mpv's `--autofit`) when previewing more than one language/contiguous run; a single window still autoplays fullscreen.
- A multi-verse extraction can show more than one distinct source caption in sequence (each verse advances its own marker's label, e.g. "Psalm 16:10" then "Psalm 16:11") - the delogo box is sized to whichever is widest/tallest across the whole window, so it stays correctly sized throughout rather than only for the first verse. The replacement reference overlay follows the same per-verse rhythm - there is never a combined "5:23-24" label; each verse gets its own drawtext switched on exactly when the source moves into it (like old `ffv`), fades matching `overlay_alpha`'s existing paragraph-boundary dip/recovery. `--slow`/`--fast` retimes those per-verse switches along with the video - a boundary landing mid-verse just splits that verse's window across the retimed cut rather than losing the switch.
...
- `--onthefly` and `--segment` both seek directly against the CDN URL, which is a byte-approximate seek over the network rather than a local frame-accurate one — fine for casual viewing, and `--segment` avoids repeating that seek on a later request for any already-cached window (not just an identical one - see the cache section above). Use `--cache` (whole chapter) if you need frame-exact boundaries, or want one download to amortize several *different* verses from the same chapter up front.
